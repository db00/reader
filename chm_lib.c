...
/***************************************************************************
  gcc -Wall chm.c chm_lib.c lzx.c && ./a.out gl2.chm /gl/glReadPixels.htm glReadBuffer.htm
		...
		Int64 len)
{
	//printf("--------------------");
	Int64 readLen=0;
	if (h->data ==  NULL || os+len>h->size){
		//printf("_chm_fetch_bytes no data!\n");
		return readLen;
	}
	memcpy((char*)buf,h->data+os,len);

	//printf("readLen:%d,position:%d\n",(int)readLen,(int)os);
	fflush(stdout);
	//CHM_RELEASE_LOCK(h->mutex);
	return len;
}

//open chm file from memory
struct chmFile *chm_mem(char *data,int data_len)
{
	...
	if (newHandle == NULL)
		return NULL;
	newHandle->data = data;
	newHandle->size = (long)data_len;
	newHandle->position = 0;
	...
	{
		//printf("_chm_fetch_bytes XXXXXXXXXXXXXXXXXXXx");
		chm_close(newHandle);
		return NULL;
	}
	if( !_unmarshal_itsf_header(&sbufpos, &sremain, &itsfHeader))
	{
		//printf("_unmarshal_itsf_header XXXXXXXXXXXXXXXXXXXx");
		chm_close(newHandle);
		return NULL;
	}
